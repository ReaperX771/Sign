...
function Bar() {
  ...
  { label: "Footer", anchor: "#footer" },
    ...
    {
      name: "Fun & Games",
      icon: <FaGamepad className="inline-block mr-2 text-white" />,
      key: "fun",
      dropdown: [
        { label: "Daily Streak", path: "/fung/stre" },
        { label: "Click to catch", path: "/fung/ora" },
        { label: "Claim window", path: "/fung/rewar" },
      ],
    },
  ];
    ...
    <section>
      <div>
        <div className="flex justify-between items-center px-4 sm:px-6 lg:px-20 py-6 bg-gradient-to-b from-[#fc9200] to-[#f72800] fixed top-0 left-0 right-0 shadow z-50">
          ...
          <nav className="hidden xl:flex xl:static xl:flex-row xl:gap-10 xl:bg-transparent xl:pt-0 xl:items-center xl:justify-start">
            {navItems.map((item) => (
              <div key={item.key} className="relative text-left">
                <button
                  onClick={() => toggleDropdown(item.key)}
                ...
                <div
                  className={`
                    ${dropdownOpen[item.key] ? "block" : "hidden"}
                    absolute bg-gray-900 shadow-lg rounded-md mt-2 w-48
                  `}
                >
                  {item.dropdown.map((subItem) =>
                    subItem.anchor ? (
